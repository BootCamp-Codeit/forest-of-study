...
| `npm run db:seed` | **데모 데이터 전체 재생성** (기존 삭제 후 아래 삽입) |

**`db:seed` 포함 데이터 (스터디 1건당):**

| 데이터 | 개수 | 설명 |
|--------|------|------|
| **STUDY** | 100건 | 닉네임·제목·소개·배경 |
| **HABIT** | 5~10개 | 현재 `WEEK_NUM` 주차, 오늘 요일 체크 상태 포함 |
| **FOCUS** | 3~8건 | 집중 시간(초) 기록 |
| **POINT_HISTORY** | 4~8건 | 포인트 이력 |
| **POINT_MASTER** | 1건 | 누적 포인트 |
| **EMOJI** | 2~4종 | 반응 카운트 |
